...
function Get-LatestRelease {
    try {
        $releases = Invoke-RestMethod -Uri 'https://api.github.com/repos/ChrisTitusTech/ATATOOLS/releases'
...
function RedirectToLatestPreRelease {
    ...
    if ($latestRelease) {
        $url = "https://github.com/ChrisTitusTech/ATATOOLS/releases/download/$latestRelease/ATATOOLS.ps1"
    } else {
        Write-Host 'Unable to determine latest pre-release version.' -ForegroundColor Red
        Write-Host "Using latest Full Release"
        $url = "https://github.com/ChrisTitusTech/ATATOOLS/releases/latest/download/ATATOOLS.ps1"
    }
