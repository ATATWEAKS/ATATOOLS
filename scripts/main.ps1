...
$functions = (Get-ChildItem function:\).where{$_.name -like "*ATATOOLS*" -or $_.name -like "*WPF*"}
...
if (-NOT ($readerOperationSuccessful)) {
    ...
    Write-Host "Quitting ATATOOLS..." -ForegroundColor Red
...
# Setup the Window to follow listen for windows Theme Change events and update the ATATOOLS theme
...
#Persist the Chocolatey preference across ATATOOLS restarts
$ChocoPreferencePath = "$env:LOCALAPPDATA\ATATOOLS\preferChocolatey.ini"
...
# Set the path for the ATATOOLS directory
$ATATOOLSdir["path"] = "$env:LOCALAPPDATA\ATATOOLS\"
[System.IO.Directory]::CreateDirectory($ATATOOLSdir["path"]) | Out-Null

$ATATOOLSdir["logo.ico"] = $ATATOOLSdir["path"] + "cttlogo.ico"

if (Test-Path $ATATOOLSdir["logo.ico"]) {
    $sync["logorender"] = $ATATOOLSdir["logo.ico"]
} else {
...
GitHub   : <a href="https://github.com/ChrisTitusTech/ATATOOLS">ChrisTitusTech/ATATOOLS</a>
Version  : <a href="https://github.com/ChrisTitusTech/ATATOOLS/releases/tag/$($sync.version)">$($sync.version)</a>
"@
