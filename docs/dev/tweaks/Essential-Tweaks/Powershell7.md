...
```json
{
  ...
  "link": "https://christitustech.github.io/ATATOOLS/dev/tweaks/Essential-Tweaks/Powershell7"
}
```
...
function Set-WinUtilProgressbar{
    <#
    .SYNOPSIS
        This function is used to Update the Progress Bar displayed in the ATATOOLS GUI.
...
function Set-WinUtilTaskbaritem {
    ...
    if ($overlay) {
        switch ($overlay) {
            'logo' {
                $sync["Form"].taskbarItemInfo.Overlay = "$env:LOCALAPPDATA\ATATOOLS\cttlogo.png"
            }
            'checkmark' {
                $sync["Form"].taskbarItemInfo.Overlay = "$env:LOCALAPPDATA\ATATOOLS\checkmark.png"
            }
            'warning' {
                $sync["Form"].taskbarItemInfo.Overlay = "$env:LOCALAPPDATA\ATATOOLS\warning.png"
            }
            'None' {
