function Invoke-WPFShortcut {
    ...
    Switch ($ShortcutToAdd) {
        "WinUtil" {
            ...
            $shellArgs = "-ExecutionPolicy Bypass -Command `"Start-Process $shell -verb runas -ArgumentList `'-Command `"irm https://github.com/ChrisTitusTech/ATATOOLS/releases/latest/download/ATATOOLS.ps1 | iex`"`'"
    ...
    if (-NOT (Test-Path -Path $ATATOOLSdir["logo.ico"])) {
        Invoke-WebRequest -Uri "https://christitus.com/images/logo-full.ico" -OutFile $ATATOOLSdir["logo.ico"]
    }
    if (Test-Path -Path $ATATOOLSdir["logo.ico"]) {
        $shortcut.IconLocation = $ATATOOLSdir["logo.ico"]
    }
