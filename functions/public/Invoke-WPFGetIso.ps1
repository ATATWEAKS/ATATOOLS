function Invoke-WPFGetIso {
    ...
    if (!$oscdImgFound) {
        ...
        if (!$downloadFromGitHub) {
            ...
            [System.Windows.MessageBox]::Show("oscdimge.exe is not found on the system, ATATOOLS will now attempt do download and install it using choco. This might take a long time.")
            ...
            [System.Windows.MessageBox]::Show("oscdimg is installed, now close, reopen PowerShell terminal and re-launch ATATOOLS.ps1")
            return
        } else {
            [System.Windows.MessageBox]::Show("oscdimge.exe is not found on the system, ATATOOLS will now attempt do download and install it from github. This might take a long time.")
    ...
    try {
        ...
        Write-Error "This is NOT ATATOOLS's problem, your ISO might be corrupt, or there is a problem on the system"
        Write-Host "Please refer to this wiki for more details: https://christitustech.github.io/ATATOOLS/KnownIssues/#troubleshoot-errors-during-microwin-usage" -ForegroundColor Red
        Set-WinUtilTaskbaritem -state "Error" -value 1 -overlay "warning"
        return
    }
